spark.sql("""--IMPORTANT: This Hive script assumes init.hql has been executed first. 
...
spark.sql("""--delete table course_activity - should already be deleted during init.hql, 
--statement here as extra protection.
DROP TABLE IF EXISTS course_activity""")

# COMMAND ----------

spark.sql("""--create table course_activity that will contain the output of this program
...
spark.sql("""--add Python UDF 'consumptionSummary.py' that generates a consumption summary for each course
ADD FILE consumptionSummary.py""")

# COMMAND ----------

spark.sql("""--using the output of a select query that returns 
...
spark.sql("""--NOTE: REMOVING THE SELECT STATEMENT BELOW FROM THIS FILE AND ADDING IT AS ANOTHER HIVE CALL INSTEAD: 
